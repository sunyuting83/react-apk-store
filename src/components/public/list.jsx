...
class List extends Component {
    ...
    getFirstData(cid, type) {
        // console.log(cid);
        let param = {
            id: cid,
            page: 1
        };
        get(type, param).then((data) => {
            if (data.length > 0) {
                this.setState({
                    data: data,
                    action: STATS.reset,
    ...
    handleAction = action => {
        ...
        if (action === STATS.refreshing) { //刷新
            setTimeout(() => {
                //refreshing complete
                this.getFirstData(this.state.cid, this.state.type);
            }, 300)
    ...
    componentDidMount() {
        if (this.contentNode) {
            ...
            this.contentNode.scrollTop = this.state.scrollTop
        };
        ...
        if (cid !== this.state.cid) {
            this.getFirstData(cid, type);
            document.getElementById("train-course").scrollTop = 0
        };
        setTimeout(() => {
            let redux = this.props.red_list_data;
            if (redux.data.length > 0 && redux.cid === this.state.cid) {
                this.setState({
                    data: redux.data,
                    page: redux.page,
                    action: STATS.reset
                });
                document.getElementById("train-course").scrollTop = redux.scrollTop
            };
        }, 50)
            ...
            <section className="content nobottom bgcolor-fff">
                <ReactPullLoad id="train-course" className="scroll-block" isBlockContainer={true} downEnough={200} action={this.state.action} handleAction={this.handleAction} hasMore={hasMore} distanceBottom={1000}>
                {
                    data.length > 0 && data.map((apk, i) => (
                        <Link to={`/item/${apk.id}`} className="row apkls" key={i} onClick={this.setData}>
